...
const types = ['date', 'datetime-local', 'email', 'number', 'password', 'range', 'search', 'tel', 'time', 'url', 'week', 'month']; // used but unnecessary leaving for future use...
...
const useForm = (initialState, formName = 'default') => {
  ...
  const mapInputs = (state, filter) => {
    ...
    if (filter) {
      const filteredState = filter.reduce((obj, key) => ({ ...obj,
        [key]: state[key]
      }), {});
    ...
    return (options = []) => {
      ...
      return stateToMap.map(([key, value], index) => {
        ...
        }, label), textarea ? React.createElement("textarea", {
